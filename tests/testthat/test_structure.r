...
test_that("correct structure", {
  ...
  expect_true(tibble::is_tibble(keyword_search(
    x = path,
...
test_that("heading search", {
  ...
  expect_true(tibble::is_tibble(heading_search(
    file,
...
test_that('ignore_case functionality', {
  ...
  expect_false(isTRUE(all.equal(nrow(r_ignore_case), nrow(R_case))))
})
